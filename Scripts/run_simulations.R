...
run_simulations <- function(generator, num_simulations) {
  ...
  all_results_em_beta_pxl <- vector("list", num_simulations)  # Store results for em_beta_pxl
  ...
  total_simulation_time <- system.time({
    for (sim in 1:num_simulations) {
      ...
      # Run and time the EM_BETA_PXL algorithm
      total_em_beta_pxl_time <- total_em_beta_pxl_time + system.time({
        results_em_beta_pxl <- run_em_algorithm(em_beta_pxl, data, q, tol = tol, 
                                                max_iter = max_iter, ll = ll, num_runs = num_runs)
      })[3]
